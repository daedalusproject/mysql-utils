...
testCreateDatabase() {
    ...
    check_database_exists=$(mysql -u$MYSQL_USER -p$MYSQL_PASSWORD -h $MYSQL_HOST -P $MYSQL_PORT -Bse "show databases" 2> /dev/null| grep "newdatabase" > /dev/null)
    check_database_error=$?
    assertEquals "0" "$check_database_error"
}

testErroredCreateDatabase() {
    MYSQL_USER="root"
    MYSQL_PASSWORD="letmein"
    MYSQL_HOST="percona-server"
    MYSQL_PORT=3306
